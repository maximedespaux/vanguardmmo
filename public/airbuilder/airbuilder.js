...
const E=s=>ST().eq[s];
function save(){try{state._ts=Date.now();localStorage.setItem('vg_air_e1',JSON.stringify(state));}catch(e){}vgPushCloud();}
var _vgPushT=null;
function vgPushCloud(){if(window.__embed)return;if(_vgPushT)clearTimeout(_vgPushT);_vgPushT=setTimeout(function(){try{fetch('/api/builder-state',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({blob:state})}).catch(function(){});}catch(e){}},1500);}
...
function render(){
  ...
  document.getElementById('stuffbar').innerHTML='<span class="slabel">Stuffs :</span>'+(C().stuffs||[]).map((s,i)=>`<div class="stab ${i===C().curStuff?'on':''}" onclick="switchStuff(${i})">${esc(s.name)}${(C().stuffs.length>1)?` <span class="x" onclick="event.stopPropagation();delStuff(${i})">✕</span>`:''}</div>`).join('')+'<div class="saddp" onclick="addStuff()">+ Stuff</div>'+((C().stuffs||[]).length?'<div class="saddp" onclick="copyStuff()" title="Crée une copie du stuff actuel (mêmes items)">📋 Dupliquer</div>':'');
  try{
