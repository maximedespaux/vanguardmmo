...
export default function EventsPage() {
  ...
  useEffect(() => { load(); }, []);
  const flash = (m: React.ReactNode) => { setToast(m); setTimeout(() => setToast(""), 2800); };
  ...
  const add = async () => {
    ...
    if (r.ok) { setF({ ...f, name: "", embedTitle: "", embedDesc: "", embedImage: "" }); load(); flash(<>Événement ajouté <Icon name="check" size={14} style={{ display: "inline-block", verticalAlign: "-2px" }} /></>); } else flash("Erreur");
  };
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {list.map((e) => (
            <div key={e.id} className="glass-card" style={{ display: "flex", alignItems: "center", gap: 14, padding: "12px 16px", opacity: e.enabled ? 1 : 0.5 }}>
              ...
              <button onClick={() => del(e.id)} title="Supprimer" style={{ ...inp, cursor: "pointer", color: "var(--red)", borderColor: "var(--border)", display: "flex", alignItems: "center", justifyContent: "center" }}><Icon name="x" size={15} /></button>
            </div>
          ))}
        </div>
      )}
