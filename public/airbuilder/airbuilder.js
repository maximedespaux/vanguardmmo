...
function pasteClip(){var clip=clipGet();if(!clip||!clip.length){agToast('Presse-papier vide.',false);return;}var s=ST();if(!s)return;s.eq=s.eq||{};clip.forEach(function(x){s.eq[x.slot]=JSON.parse(JSON.stringify(x.e));});render();if(typeof save==='function')save();agToast(clip.length+' item(s) collé(s) sur « '+(s.name||'')+' » ✓',true);}
