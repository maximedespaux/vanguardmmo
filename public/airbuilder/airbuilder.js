...
function vgSavePersos(){if(window.__VIEW){agToast('Mode lecture seule — build d\'un autre membre.',false);return;}agConfirm('💾 Sauvegarder TOUS tes persos sur le site ?\n\n⚠️ Ça remplace tes persos déjà enregistrés (tous les stuffs).',function(){fetch('/api/builder-state',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({blob:state,snapshot:true})}).catch(function(){});fetch('/api/characters/sync',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({chars:vgCollectBuilds()})}).then(function(r){return r.ok?r.json():Promise.reject(r);}).then(function(d){agToast('✅ '+((d&&d.count)||'Tes')+' perso(s) sauvegardé(s) — visibles dans le GuildViewer / Compositions / Dashboard.',true);}).catch(function(){agToast('❌ Erreur de sauvegarde — es-tu bien connecté au site ?',false);});});}
