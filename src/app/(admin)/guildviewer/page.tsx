...
export default function GuildViewerPage() {
  ...
  const miniStat = (ic: IconName, n: number, l: string, c: string) => (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", minWidth: 50, background: "var(--bg-3)", border: "1px solid var(--border)", borderRadius: 9, padding: "5px 9px" }}>
      ...
      <span style={{ display: "inline-flex", alignItems: "center", gap: 3, fontSize: 9.5, color: "var(--text-muted)", marginTop: 2, whiteSpace: "nowrap" }}><Icon name={ic} size={11} />{l}</span>
    </div>
  );
            ...
            <div key={u.id} className="gv-member glass-card" style={{ padding: 0, marginBottom: 13, overflow: "hidden", borderLeft: `3px solid ${r.color}` }}>
              ...
              {histUser === u.id && (
                <div style={{ margin: "0 17px 13px", padding: "11px 13px", background: "var(--bg-3)", border: "1px solid var(--border)", borderRadius: 10 }}>
                  <div className="font-heading" style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, textTransform: "uppercase", letterSpacing: 1, color: "var(--text-muted)", marginBottom: 8 }}><Icon name="clock" size={13} />Historique des builds</div>
                  {histLoading ? <div style={{ fontSize: 12, color: "var(--text-muted)" }}>Chargement…</div>
                    : hist.length === 0 ? <div style={{ fontSize: 12, color: "var(--text-muted)" }}>Aucune version archivée — les versions se créent automatiquement quand le membre édite son build.</div>
              ...
              {/* Personnages */}
              {noChar ? (
                <div style={{ margin: "0 17px 15px", padding: "11px 14px", background: "rgba(255,210,74,.06)", border: "1px dashed var(--gold)", borderRadius: 10, fontSize: 12.5, color: "var(--gold)" }}><Icon name="alert" size={14} style={{ verticalAlign: "-2px", marginRight: 6 }} />Aucun personnage déclaré — à inviter à compléter son profil via le Builder.</div>
              ) : (
                    ...
                    return (
                      <div key={c.id} className="gv-char" style={{ display: "flex", alignItems: "center", gap: 11, flexWrap: "wrap", background: "var(--bg-3)", border: "1px solid var(--border)", borderRadius: 11, padding: "9px 13px" }}>
                        ...
                        <div style={{ minWidth: 130 }}>
                          <div style={{ fontSize: 13.5, fontWeight: 600 }}>{c.name} {c.isMain && <span title="Personnage principal" style={{ display: "inline-flex", verticalAlign: "-2px", color: "var(--gold)" }}><Icon name="star" size={12} /></span>}</div>
                        ...
                        <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center", marginLeft: "auto" }}>
                          ...
                          {c.gearProfiles.length > 0 && <a href={`/builder/${u.id}`} target="_blank" rel="noopener noreferrer" style={{ fontSize: 10.5, fontWeight: 700, color: "var(--orange)", background: "rgba(255,140,26,.1)", border: "1px solid var(--orange)", borderRadius: 6, padding: "3px 10px", textDecoration: "none", whiteSpace: "nowrap", display: "inline-flex", alignItems: "center", gap: 5 }}><Icon name="sword" size={12} />AirBuilder ↗</a>}
                        </div>
                      </div>
                    );
                  })}
