...
function render(){
  ...
  document.getElementById('stuffbar').innerHTML='<span class="slabel">Stuffs :</span>'+(C().stuffs||[]).map((s,i)=>`<div class="stab ${i===C().curStuff?'on':''}" onclick="switchStuff(${i})" ondblclick="event.stopPropagation();renStuff(${i})" title="Clic : ouvrir · Double-clic : renommer">${esc(s.name)}${(C().stuffs.length>1)?` <span class="x" onclick="event.stopPropagation();delStuff(${i})">✕</span>`:''}</div>`).join('')+'<div class="saddp" onclick="addStuff()">+ Stuff</div>'+((C().stuffs||[]).length?'<div class="saddp" onclick="copyStuff()" title="Crée une copie complète du stuff actuel (mêmes items)">📋 Dupliquer</div>':'')+((C().stuffs||[]).length?'<div class="saddp" onclick="copySheet()" title="Copier certains items (avec leurs modifications) pour les coller sur un autre stuff">📑 Copier items</div>':'')+((clipGet()&&clipGet().length)?'<div class="saddp" onclick="pasteClip()" title="Coller ici les items copiés" style="border-color:#ff8c1a;color:#ff8c1a;font-weight:700">📥 Coller ('+clipGet().length+')</div>':'');
  try{
