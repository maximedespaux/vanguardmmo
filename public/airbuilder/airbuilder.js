...
var _vgPushT=null;
function vgPushCloud(){if(window.__embed||window.__VIEW)return;if(_vgPushT)clearTimeout(_vgPushT);_vgPushT=setTimeout(function(){try{fetch('/api/builder-state',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({blob:state})}).catch(function(){});}catch(e){}},1500);}
...
function addStuff(){const dn='Stuff '+((C().stuffs||[]).length+1);agPrompt('Nom du nouveau stuff ?',dn,function(n){if(n===null)return;C().stuffs.push(newStuff((n||'').trim()||dn));C().curStuff=C().stuffs.length-1;render();});}
...
function pasteClip(){var clip=clipGet();if(!clip||!clip.length){agToast('Presse-papier vide.',false);return;}var s=ST();if(!s)return;var c=C();
...
function vgSaveHelp(){agInfo('💾 Comment marche la sauvegarde de l\'AirBuilder ?\n\n• SAUVEGARDE AUTOMATIQUE : chaque changement (équipement, stuff, perso) est enregistré tout seul. Aucun bouton à cliquer pour ne rien perdre.\n\n• ⚠️ MAIS chaque changement ÉCRASE le précédent : pas d\'annulation. Si tu vides ou rééquipes un stuff, l\'ancien état est remplacé définitivement.\n\n• 📤 « Publier mes persos » : envoie tes builds à la guilde (GuildViewer · Compositions · Dashboard) ET crée une VERSION restaurable (les 10 dernières sont gardées).\n\n👉 Avant un gros remaniement, clique « Publier » : tu te gardes un point de restauration.');}
