...
function viewBank(){
  ...
  return `<div class="card"><div class="sec-h">🏦 Coffres <span class="n">commun + individuels</span></div><div class="mtabs">${mtabs}</div></div>
   <div class="toolbar" style="margin-top:14px"><input class="inp" id="bankq" placeholder="Rechercher un objet…" value="${esc(bankQ)}" oninput="bankQ=this.value;filterBank(this.value)" style="flex:1;min-width:180px">${canEdit()?'<button class="btn o" onclick="addItem()">＋ Objet</button>':''}<button class="btn" onclick="recapSheet()">📊 Récap</button><button class="btn" onclick="openJournal()">🧾 Journal</button></div>
   <div id="bankbody">${bankBody()}</div>`;
}
// ── B1 : récap « Mes objets / Mes ventes » + qui détient quoi (par membre) ──
function recapSheet(){var me=(window.__agUser||'').toLowerCase().trim();var members=S.members||[];var nameOf={};(catalog()||[]).forEach(function(it){nameOf[it.id]=it;});
  ...
  openSheet('<h3>📊 Récap des coffres — qui détient quoi</h3><div class="hint">Le contenu de chaque coffre, membre par membre. Le tien (« 🫵 Mes objets ») est surligné.</div>'+body+'<div class="toolbar" style="justify-content:flex-end;margin:0"><button class="btn" onclick="closeSheet()">Fermer</button></div>');}
