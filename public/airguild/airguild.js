...
function viewDj(){
  ...
  return `${farmBar()}
   <div class="toolbar"><input class="inp" placeholder="<i class=vgi-search></i> Cherche un objet (Yggdrasil, Anneau, Nucléus…) ou un donjon" value="${esc(q)}" oninput="q=this.value;render()" style="flex:1;min-width:240px"><div class="seg">${['Tous','SOLO','GROUPE'].map(t=>`<button class="${djType===t?'on':''}" onclick="djType='${t}';render()">${t==='Tous'?'Tous':t==='SOLO'?'<i class=vgi-sun></i> Solo':'<i class=vgi-users></i> Groupe'}</button>`).join('')}</div><span class="pill">${matching.length}/${D.dungeons.length}</span></div>
...
function openFarm(){const keys=Object.keys(S.farm);openSheet(`<h3><i class=vgi-target></i> Objectifs de farm</h3><div class="hint">« Possédé » se verse dans <b>ton coffre</b> (qui compte dans le Total guilde) puis se remet à 0.</div>${keys.length?keys.map(k=>{const f=S.farm[k];const dj=D.dungeons.find(x=>x.id===f.dj);const pc=f.target?Math.min(100,f.have/f.target*100):0;
   return `<div class="fitem">${img(f.ic)||'<span class="x" style="width:30px;height:30px;display:flex;align-items:center;justify-content:center;background:#ffffff08;border-radius:8px"><i class=vgi-package></i></span>'}<div class="fn"><div class="a">${esc(f.n)}</div><div class="b">${dj?dj.icon+' '+esc(dj.name):''}</div><div class="prog"><i style="width:${pc}%"></i></div></div>
