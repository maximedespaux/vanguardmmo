...
function render(){
  ...
  document.getElementById('stuffbar').innerHTML='<span class="slabel">Stuffs :</span>'+(C().stuffs||[]).map((s,i)=>`<div class="stab ${i===C().curStuff?'on':''}" onclick="switchStuff(${i})" ondblclick="event.stopPropagation();renStuff(${i})" title="Clic : ouvrir · Double-clic : renommer">${esc(s.name)}${(C().stuffs.length>1)?` <span class="x" onclick="event.stopPropagation();delStuff(${i})">✕</span>`:''}</div>`).join('')+'<div class="saddp" onclick="addStuff()">+ Stuff</div>'+((C().stuffs||[]).length?'<div class="saddp" onclick="copyStuff()" title="Crée une copie complète du stuff actuel (mêmes items)">📋 Dupliquer</div>':'')+((C().stuffs||[]).length?'<div class="saddp" onclick="copySheet()" title="Copier certains items (avec leurs modifications) pour les coller sur un autre stuff">📑 Copier items</div>':'')+((C().stuffs||[]).length?'<div class="saddp" onclick="maximizeAll()" title="Met tous les niveaux au max sur ce stuff (tu gardes le choix des éléments / stats)">🎯 Tout maximiser</div>':'')+'<div class="saddp" onclick="templatesMenu()" title="Enregistrer / appliquer des modèles de stuff réutilisables">🧩 Modèles'+(templates().length?' ('+templates().length+')':'')+'</div>'+((clipGet()&&clipGet().length)?'<div class="saddp" onclick="pasteClip()" title="Coller ici les items copiés" style="border-color:#ff8c1a;color:#ff8c1a;font-weight:700">📥 Coller ('+clipGet().length+')</div>':'');
  try{
...
function applyTemplate(idx){if(_ro())return;var t=templates()[idx];if(!t)return;var c=C();var ok=[],skip=0;Object.keys(t.eq||{}).forEach(function(slot){var e=t.eq[slot];if(!e||!e.item)return;var id=e.item.id;var lst=listFor(slot)||[];var valid=(lst.length===0)||lst.some(function(it){return String(it.id)===String(id);});if(valid)ok.push({slot:slot,e:e});else skip++;});
  ...
  agConfirm('Appliquer le modèle « '+t.name+' » sur « '+(ST().name||'Stuff')+' » ?\nÇa remplace les pièces concernées.'+(skip?'\n\n⚠️ '+skip+' pièce(s) ignorée(s) (incompatible(s) avec '+(c.cls||'')+' '+(c.sex||'')+').':''),function(){var st=ST();st.eq=st.eq||{};ok.forEach(function(x){st.eq[x.slot]=JSON.parse(JSON.stringify(x.e));});render();if(typeof save==='function')save();agToast('Modèle appliqué ✓'+(skip?' · '+skip+' ignorée(s)':''),true);});}
...
function vgShare(){if(window.__VIEW)return;
  fetch('/api/builder-share').then(function(r){return r.ok?r.json():null;}).then(function(s){
    var link=(s&&s.shareId)?(location.origin+'/build/'+s.shareId):'';var pub=!!(s&&s.public);
    document.getElementById('modalRoot').innerHTML='<div class="modal" onclick="if(event.target===this)closePick()"><div class="sheet" style="max-width:480px"><h3>🔗 Partager mon build</h3>'
     +'<div class="mini" style="margin-bottom:12px">Un lien vers ton build. En <b>public</b>, n\'importe qui avec le lien peut le consulter (lecture seule), comme DofusBook. En privé, le lien ne fonctionne plus.</div>'
     +'<div class="row" style="gap:8px;margin-bottom:12px"><input id="__shlink" class="srch" style="flex:1" readonly value="'+(link||'— active le partage pour générer le lien —')+'"><span class="pill" onclick="vgShareCopy()">📋 Copier</span></div>'
     +'<div class="row" style="gap:10px;justify-content:space-between;align-items:center"><label class="mini" style="display:flex;align-items:center;gap:7px;cursor:pointer"><input type="checkbox" id="__shpub" '+(pub?'checked':'')+'> Public (visible via le lien)</label>'
     +'<span class="pill" style="background:#FF8C1A;color:#0A0A0C;font-weight:700" onclick="vgShareSave()">'+(link?'Mettre à jour':'Activer le partage')+'</span></div>'
     +'<div class="toolbar" style="justify-content:flex-end;margin:14px 0 0"><span class="pill" onclick="closePick()">Fermer</span></div></div></div>';
  }).catch(function(){agToast('Erreur de partage.',false);});}
function vgShareSave(){var pub=document.getElementById('__shpub').checked;fetch('/api/builder-share',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({public:pub})}).then(function(r){return r.ok?r.json():null;}).then(function(s){if(!s)return agToast('Erreur.',false);var i=document.getElementById('__shlink');if(i)i.value=location.origin+'/build/'+s.shareId;agToast(pub?'Partage public activé ✓':'Partage passé en privé.',true);}).catch(function(){agToast('Erreur.',false);});}
function vgShareCopy(){var i=document.getElementById('__shlink');if(!i||!i.value||i.value.charAt(0)==='—')return agToast('Active d\'abord le partage.',false);try{navigator.clipboard.writeText(i.value);}catch(e){try{i.select();document.execCommand('copy');}catch(e2){}}agToast('Lien copié ✓',true);}
