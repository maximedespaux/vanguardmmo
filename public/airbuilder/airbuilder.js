...
function render(){
  ...
  document.getElementById('stuffbar').innerHTML='<span class="slabel">Stuffs :</span>'+(C().stuffs||[]).map((s,i)=>`<div class="stab ${i===C().curStuff?'on':''}" onclick="switchStuff(${i})">${esc(s.name)}${(C().stuffs.length>1)?` <span class="x" onclick="event.stopPropagation();delStuff(${i})">✕</span>`:''}</div>`).join('')+'<div class="saddp" onclick="addStuff()">+ Stuff</div>'+((C().stuffs||[]).length?'<div class="saddp" onclick="copyStuff()" title="Crée une copie complète du stuff actuel (mêmes items)">📋 Dupliquer</div>':'')+((C().stuffs||[]).length?'<div class="saddp" onclick="copySheet()" title="Copier certains items (avec leurs modifications) pour les coller sur un autre stuff">📑 Copier items</div>':'')+((clipGet()&&clipGet().length)?'<div class="saddp" onclick="pasteClip()" title="Coller ici les items copiés" style="border-color:#ff8c1a;color:#ff8c1a;font-weight:700">📥 Coller ('+clipGet().length+')</div>':'');
  try{
...
function copyStuff(){var c=C();var s=ST();if(!s)return;var copy={name:((s.name||'Stuff')+' (copie)').slice(0,40),eq:JSON.parse(JSON.stringify(s.eq||{}))};c.stuffs.push(copy);c.curStuff=c.stuffs.length-1;render();agToast('Stuff dupliqué ✓ — renomme-le et ajuste',true);}
// ── Copier des items précis (avec leurs modifications) vers un autre stuff ──
function copySheet(){var s=ST();var eq=(s&&s.eq)||{};var slots=Object.keys(eq).filter(function(k){return eq[k]&&eq[k].item;});if(!slots.length){agToast('Aucun item équipé à copier sur ce stuff.',false);return;}var rows=slots.map(function(k){var it=eq[k].item;var sl=SLOTS[k]||{lbl:k,ico:'•'};return '<label class="itl" style="cursor:pointer;align-items:center"><input type="checkbox" class="__cpk" value="'+esc(k)+'" checked style="margin-right:10px;width:16px;height:16px;accent-color:#ff8c1a;flex:none"><span style="font-size:18px;margin-right:8px">'+((sl.ico)||'•')+'</span><div class="n"><b>'+esc(sl.lbl)+'</b><div style="font-size:10px;color:var(--mut)">'+esc(it.n||'')+'</div></div></label>';}).join('');document.getElementById('modalRoot').innerHTML='<div class="modal" onclick="if(event.target===this)closePick()"><div class="sheet"><h3>📑 Copier des items</h3><div class="mini" style="margin-bottom:10px">Coche les items à copier (avec toutes leurs modifications : sertissage, perçage, éveil…), puis va sur un autre stuff et clique « Coller ».</div><div style="display:flex;gap:8px;margin-bottom:8px"><span class="pill" onclick="cpkAll(1)">Tout cocher</span><span class="pill" onclick="cpkAll(0)">Tout décocher</span></div><div style="max-height:320px;overflow:auto;display:flex;flex-direction:column;gap:5px">'+rows+'</div><div class="sheet-foot"><span class="pill" onclick="copyDo()" style="background:#ff8c1a;color:#0A0A0C;font-weight:700">📋 Copier la sélection</span><span class="pill" onclick="closePick()">Fermer</span></div></div></div>';}
function cpkAll(v){[].slice.call(document.querySelectorAll('.__cpk')).forEach(function(i){i.checked=!!v;});}
function copyDo(){var s=ST();var eq=(s&&s.eq)||{};var picked=[].slice.call(document.querySelectorAll('.__cpk:checked')).map(function(i){return i.value;});if(!picked.length){agToast('Coche au moins un item.',false);return;}var clip=picked.map(function(k){return {slot:k,e:JSON.parse(JSON.stringify(eq[k]))};}).filter(function(x){return x.e&&x.e.item;});window.__vgClip=clip;try{localStorage.setItem('vg_air_clip',JSON.stringify(clip));}catch(e){}document.getElementById('modalRoot').innerHTML='';render();agToast(clip.length+' item(s) copié(s) — va sur un autre stuff et clique « Coller ».',true);}
function clipGet(){if(window.__vgClip&&window.__vgClip.length)return window.__vgClip;try{var c=JSON.parse(localStorage.getItem('vg_air_clip')||'null');if(c&&c.length){window.__vgClip=c;return c;}}catch(e){}return null;}
function pasteClip(){if(_ro())return;var clip=clipGet();if(!clip||!clip.length){agToast('Presse-papier vide.',false);return;}var s=ST();if(!s)return;var c=C();
  // #11 : on ne colle que les items équipables par ce perso (prérequis classe + sexe via listFor ; slots neutres = familier/fée = toujours OK)
  var ok=[],skip=0;clip.forEach(function(x){var id=x.e&&x.e.item&&x.e.item.id;var lst=listFor(x.slot)||[];var valid=(lst.length===0)||lst.some(function(it){return String(it.id)===String(id);});if(valid)ok.push(x);else skip++;});
  if(!ok.length){agToast('Aucun item compatible avec '+(c.cls||'cette classe')+' '+(c.sex||'')+'.',false);return;}
  var msg='Coller '+ok.length+' item(s) sur « '+(s.name||'Stuff')+' » ?\nÇa remplace les pièces concernées.'+(skip?'\n\n⚠️ '+skip+' item(s) ignoré(s) (incompatible(s) avec '+(c.cls||'')+' '+(c.sex||'')+').':'');
  agConfirm(msg,function(){var st=ST();st.eq=st.eq||{};ok.forEach(function(x){st.eq[x.slot]=JSON.parse(JSON.stringify(x.e));});window.__vgClip=null;try{localStorage.removeItem('vg_air_clip');}catch(e){}render();if(typeof save==='function')save();agToast(ok.length+' item(s) collé(s)'+(skip?' · '+skip+' ignoré(s)':'')+' ✓',true);});}
...
function charForm(){if(_ro())return;window.__cfCls='Arcaniste';window.__cfSex='G';document.getElementById('modalRoot').innerHTML=`<div class="modal" onclick="if(event.target===this)agClose(0)"><div class="sheet" id="cfSheet" style="max-width:540px;padding:16px 18px"><style>#cfSheet input{width:100%;box-sizing:border-box;background:#0f0f15;border:1px solid rgba(255,255,255,.10);border-radius:8px;padding:7px 10px;color:#ece9f1;font-family:inherit;font-size:13px;font-weight:500;outline:none;box-shadow:none;-webkit-appearance:none;-moz-appearance:textfield;appearance:none;transition:border-color .14s,box-shadow .14s}#cfSheet input::placeholder{color:#6f6f78}#cfSheet input:focus{border-color:#FF8C1A;background:#14141b;box-shadow:0 0 0 3px rgba(255,140,26,.15)}#cfSheet input[type=number]::-webkit-inner-spin-button,#cfSheet input[type=number]::-webkit-outer-spin-button{-webkit-appearance:none;margin:0}#cfSheet label{display:block;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.4px;color:#9a9aa3;margin-bottom:4px}#cfSheet .cfsx{display:inline-flex;align-items:center;justify-content:center;gap:6px;padding:8px 12px;border-radius:8px;border:1px solid rgba(255,255,255,.12);background:#0f0f15;color:#d2d2d9;font-size:13px;font-weight:600;cursor:pointer;transition:border-color .14s,background .14s,color .14s;user-select:none}#cfSheet .cfsx:hover{border-color:rgba(255,255,255,.25);color:#fff}#cfSheet .cfsx.on{background:linear-gradient(180deg,#FFB552,#FF8C1A);border-color:#FF8C1A;color:#0A0A0C}#cfSheet .cfsx .sx{font-size:14px;line-height:1}</style><div style="font-weight:700;font-size:15px;margin-bottom:9px">🧙 Nouveau personnage</div><div style="display:flex;gap:14px;flex-wrap:wrap"><div style="flex:1;min-width:210px"><div class="f" style="margin-bottom:7px"><label>Classe</label><div class="clssel" id="cfClsSel">${CLASSES.map(x=>`<div class="ci ${x==='Arcaniste'?'on':''}" data-c="${x}" title="${x}" onclick="cfPickCls('${x}')">${IC['class_'+x.toLowerCase()]?`<img src="${IC['class_'+x.toLowerCase()]}">`:x[0]}</div>`).join('')}</div><div id="cfClsRecap" style="font-size:11px;color:var(--orange,#ff8c1a);margin-top:5px;font-weight:600">✓ Classe : Arcaniste</div></div><div class="f" style="margin-bottom:8px"><label>Nom du personnage</label><input class="inp" id="cfName" placeholder="ex. Daiisuke" style="width:100%"></div><div class="f" style="margin-bottom:8px"><label>Sexe</label><div style="display:flex;gap:7px"><span class="cfsx on" data-s="G" onclick="cfPickSex('G')" style="flex:1"><span class="sx">♂</span> Garçon</span><span class="cfsx" data-s="F" onclick="cfPickSex('F')" style="flex:1"><span class="sx">♀</span> Fille</span></div></div><div style="display:flex;gap:10px;flex-wrap:wrap"><div class="f" style="flex:1;min-width:80px"><label>Niveau</label><input class="inp" id="cfLvl" type="number" value="200" min="1" max="200" style="width:100%"></div><div class="f" style="flex:1;min-width:80px"><label>Prestige</label><input class="inp" id="cfPrest" type="number" value="3" min="0" max="12" style="width:100%"></div></div></div><div style="width:132px;flex:none;display:flex;flex-direction:column;align-items:center;justify-content:center;background:#ffffff06;border:1px solid #ffffff10;border-radius:10px;padding:7px;min-height:150px"><img id="cfRender" alt="" style="max-width:100%;max-height:158px;object-fit:contain"><div class="mut" style="font-size:9px;margin-top:3px;text-align:center">aperçu</div></div></div><div style="display:flex;gap:8px;justify-content:flex-end;margin-top:11px"><span class="pill" onclick="agClose(0)">Annuler</span><span class="pill" style="background:var(--orange,#ff8c1a);color:#0A0A0C;font-weight:700" onclick="doCharForm()">Créer le perso</span></div></div></div>`;var n=document.getElementById('cfName');if(n)n.focus();cfUpdateRender();}
function cfUpdateRender(){var cls=window.__cfCls||'Arcaniste',sex=window.__cfSex||'G';var im=document.getElementById('cfRender');if(im){var src=(typeof CHARIMG!=='undefined'&&(CHARIMG[cls+'|'+sex]||CHARIMG[cls+'|G']))||'';im.src=src;im.style.display=src?'block':'none';}var r=document.getElementById('cfClsRecap');if(r)r.textContent='✓ Classe : '+cls+' '+(sex==='F'?'♀':'♂');}
...
function setCls(v){if(_ro())return;if(C().cls===v)return;const hasGear=(C().stuffs||[]).some(s=>s.eq&&Object.keys(s.eq).length);const apply=function(){C().cls=v;(C().stuffs||[]).forEach(s=>s.eq={});render();};if(hasGear)agConfirm('⚠️ Changer de classe va vider tout l\'équipement de ce personnage (sur tous ses stuffs).\n\nCette action est irréversible. Continuer ?',apply);else apply();}
...
let pickSlot=null;
function _ro(){if(window.__VIEW){if(!_ro._t||Date.now()-_ro._t>1500){_ro._t=Date.now();agToast('👁️ Mode lecture — build non modifiable.',false);}return true;}return false;}
...
function itipBuild(it,cfg,slot,e){const col=it.col||'#cfd2dc';const icon=(it.ic&&IC[it.ic])?`<img src="${IC[it.ic]}">`:'';
  ...
  if(configured){let cf='';const chips=[];
    ...
    if(slot==='familier'&&e){chips.push(`<span class="chip" style="color:var(--gold);border-color:#FFD24A55">🏅 Rang ${e.rank||'D'}</span>`);if(typeof e.rune==='string'&&e.rune){const _rc=STATCOL[e.rune]||'#4ADE80';chips.push(`<span class="chip" style="color:${_rc};border-color:${_rc}55">● ${esc(e.rune)}</span>`);}}
...
function drawPick(q){itipHide();const slot=pickSlot,e=E(slot),cfg=SLOTS[slot]||{lbl:slot};
  ...
  if(slot==='familier'){
    ...
    document.getElementById('modalRoot').innerHTML=`<div class="modal" onclick="if(event.target===this)closePick()"><div class="sheet"><h3>Familier</h3><input class="srch" placeholder="🔍 Familier…" oninput="drawPick(this.value)" ${q?`value="${esc(q)}"`:''}>${pk}<div class="sheet-foot">${e?'<span class="pill rm" onclick="removeItem()">🗑️ Retirer</span>':'<span></span>'}<span class="pill" onclick="closePick()">Fermer</span></div></div></div>`;vgDD();return;}
...
function vgSaveHelp(){agInfo('💾 Comment marche la sauvegarde de l\'AirBuilder ?\n\n• SAUVEGARDE AUTOMATIQUE : chaque changement est enregistré tout seul. Aucun bouton à cliquer pour ne rien perdre.\n\n• ⚠️ Chaque changement ÉCRASE le précédent : pas d\'annulation (si tu vides ou rééquipes un stuff, l\'ancien état est remplacé).\n\n• 🕘 VERSIONS AUTOMATIQUES : un point de restauration est créé tout seul régulièrement (≈ toutes les 10 min d\'édition). Tu peux en restaurer une via le GuildViewer.\n\n• 📤 PUBLICATION AUTOMATIQUE : tes persos sont envoyés à la guilde (GuildViewer · Compositions · Dashboard) tout seuls — plus besoin de cliquer. Le bouton « Publier maintenant » force juste une mise à jour immédiate.');}
...
function panelFor(slot,e){
  ...
  if(slot==='mantra'||slot==='masque'){const b=(e.item.b||[]).map(x=>esc(x[0])+' +'+x[1]).join(' · ');return `<div class="wp"><div class="grp"><div class="gh">${slot==='mantra'?'📜 Mantra':'🎭 Masque'} ${esc(e.item.cat||'')}${e.item.lvl?' '+e.item.lvl:''}</div><div class="cb" style="color:var(--green);font-size:11px">${b||'—'}</div></div>${slot==='masque'?eveilUI('masque'):''}</div>`;}
  return '';
}
...
function capePanel(e){const b=(e.item.b||[]).map(x=>esc(x[0])+' +'+x[1]).join(' · ');return `<div class="wp"><div class="grp"><div class="gh">🧥 ${esc(e.item.n)}</div><div class="cb" style="color:var(--green);font-size:11px">${b||'—'}</div></div>${runeUI('cape')}${eveilUI('cape')}</div>`;}
function ramasseurPanel(e){const b=(e.item.b||[]).map(x=>esc(x[0])+' +'+x[1]).join(' · ');return `<div class="wp"><div class="grp"><div class="gh">🐾 ${esc(e.item.n)}</div><div class="cb" style="color:var(--green);font-size:11px">${b||'—'}</div></div>${runeUI('ramasseur')}</div>`;}
