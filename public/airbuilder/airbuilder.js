...
const E=s=>ST().eq[s];
function save(){if(window.__VIEW)return;try{state._ts=Date.now();localStorage.setItem('vg_air_e1',JSON.stringify(state));}catch(e){}vgPushCloud();}
var _vgPushT=null;
...
function charForm(){if(_ro())return;window.__cfCls='Arcaniste';window.__cfSex='G';document.getElementById('modalRoot').innerHTML=`<div class="modal" onclick="if(event.target===this)agClose(0)"><div class="sheet" style="max-width:450px;padding:22px"><div style="font-weight:700;font-size:16px;margin-bottom:14px">🧙 Nouveau personnage</div><div class="f" style="margin-bottom:12px"><label>Classe</label><div class="clssel" id="cfClsSel">${CLASSES.map(x=>`<div class="ci ${x==='Arcaniste'?'on':''}" data-c="${x}" title="${x}" onclick="cfPickCls('${x}')">${IC['class_'+x.toLowerCase()]?`<img src="${IC['class_'+x.toLowerCase()]}">`:x[0]}</div>`).join('')}</div></div><div class="f" style="margin-bottom:12px"><label>Nom du personnage</label><input class="inp" id="cfName" placeholder="ex. Daiisuke" style="width:100%"></div><div style="display:flex;gap:12px;margin-bottom:14px;flex-wrap:wrap"><div class="f"><label>Sexe</label><div style="display:flex;gap:6px"><span class="pill cfsx on" data-s="G" onclick="cfPickSex('G')">♂ G</span><span class="pill cfsx" data-s="F" onclick="cfPickSex('F')">♀ F</span></div></div><div class="f" style="flex:1;min-width:90px"><label>Niveau</label><input class="inp" id="cfLvl" type="number" value="200" min="1" max="200" style="width:100%"></div><div class="f" style="flex:1;min-width:90px"><label>Prestige</label><input class="inp" id="cfPrest" type="number" value="3" min="0" max="12" style="width:100%"></div></div><div style="display:flex;gap:10px;justify-content:flex-end"><span class="pill" onclick="agClose(0)">Annuler</span><span class="pill" style="background:var(--orange,#ff8c1a);color:#0A0A0C;font-weight:700" onclick="doCharForm()">Créer le perso</span></div></div></div>`;var n=document.getElementById('cfName');if(n)n.focus();}
function cfPickCls(x){window.__cfCls=x;var s=document.getElementById('cfClsSel');if(s)Array.prototype.forEach.call(s.children,function(d){d.classList.toggle('on',d.getAttribute('data-c')===x);});}
function cfPickSex(x){window.__cfSex=x;Array.prototype.forEach.call(document.querySelectorAll('.cfsx'),function(d){d.classList.toggle('on',d.getAttribute('data-s')===x);});}
