...
function panelFor(slot,e){
  ...
  if(slot==='mantra'||slot==='masque'){const b=(e.item.b||[]).map(x=>esc(x[0])+' +'+x[1]).join(' · ');return `<div class="grp"><div class="gh">${slot==='mantra'?'📜 Mantra':'🎭 Masque'} ${esc(e.item.cat||'')}${e.item.lvl?' '+e.item.lvl:''}</div><div class="cb" style="color:var(--green);font-size:11px">${b||'—'}</div></div>`;}
  return '';
}
