...
function render(){
  ...
  document.getElementById('stuffbar').innerHTML='<span class="slabel">Stuffs :</span>'+(C().stuffs||[]).map((s,i)=>`<div class="stab ${i===C().curStuff?'on':''}" onclick="switchStuff(${i})" ondblclick="event.stopPropagation();renStuff(${i})" title="Clic : ouvrir · Double-clic : renommer">${esc(s.name)}${(C().stuffs.length>1)?` <span class="x" onclick="event.stopPropagation();delStuff(${i})"><i class=vgi-x></i></span>`:''}</div>`).join('')+'<div class="saddp" onclick="addStuff()">+ Stuff</div>'+((C().stuffs||[]).length?'<div class="saddp" onclick="copyStuff()" title="Crée une copie complète du stuff actuel (mêmes items)"><i class=vgi-clipboard></i> Dupliquer</div>':'')+((C().stuffs||[]).length?'<div class="saddp" onclick="copySheet()" title="Copier certains items (avec leurs modifications) pour les coller sur un autre stuff"><i class=vgi-layers></i> Copier items</div>':'')+((C().stuffs||[]).length?'<div class="saddp" onclick="maximizeAll()" title="Met tous les niveaux au max sur ce stuff (tu gardes le choix des éléments / stats)"><i class=vgi-target></i> Tout maximiser</div>':'')+'<div class="saddp" onclick="templatesMenu()" title="Enregistrer / appliquer des modèles de stuff réutilisables"><i class=vgi-puzzle></i> Modèles'+(templates().length?' ('+templates().length+')':'')+'</div>'+((clipGet()&&clipGet().length)?'<div class="saddp" onclick="pasteClip()" title="Coller ici les items copiés" style="border-color:#ff8c1a;color:#ff8c1a;font-weight:700"><i class=vgi-download></i> Coller ('+clipGet().length+')</div>':'');
  try{
...
function itipBuild(it,cfg,slot,e){const col=it.col||'#cfd2dc';const icon=(it.ic&&IC[it.ic])?`<img src="${IC[it.ic]}">`:'';
  ...
  if(configured){let cf='';const chips=[];
    if(isW&&cfg)chips.push(`<span class="chip" style="${cfg.rune?'color:var(--green);border-color:#4ADE8055':'color:var(--mut)'}">${cfg.rune?'<i class=vgi-rune></i>':'<i class=vgi-awaken></i>'} ${esc(runeName(it.tier))}</span>`);
...
function drawPick(q){itipHide();const slot=pickSlot,e=E(slot),cfg=SLOTS[slot]||{lbl:slot};
  ...
  if(slot==='familier'){
    ...
    if(e){const rank=e.rank||'D';pk+=`<div class="sec"><div style="font-size:11px;color:var(--mut);text-transform:uppercase;letter-spacing:1px;margin-bottom:4px">Rang du familier (apparence change en B puis S)</div>
      ...
      <div style="font-size:11px;color:var(--mut);margin-bottom:4px">Rune (bonus)</div><div class="seg2" style="flex-wrap:wrap;max-width:340px">${['Force','Endurance','Dextérité','Intelligence'].map(o=>statS2(o,(typeof e.rune==='string'&&e.rune===o),`famRune('${o}')`)).join('')}<div class="s2 ${(typeof e.rune!=='string'||!e.rune)?'on':''}" onclick="famRune('')">—</div></div><div class="gemlbl"><i class=vgi-gem></i> Gemmes</div></div>`;}
    ...
    document.getElementById('modalRoot').innerHTML=`<div class="modal" onclick="if(event.target===this)closePick()"><div class="sheet"><h3>Familier</h3><input class="srch" placeholder="<i class=vgi-search></i> Familier…" oninput="drawPick(this.value)" ${q?`value="${esc(q)}"`:''}>${pk}<div class="sheet-foot">${e?'<span class="pill rm" onclick="removeItem()"><i class=vgi-trash></i> Retirer</span>':'<span></span>'}<span class="pill" onclick="closePick()">Fermer</span></div></div></div>`;vgDD();return;}
  ...
  const pick=`<input class="srch" placeholder="<i class=vgi-search></i> ${items.length} objet(s) pour ${esc(C().cls)} ${C().sex}…" oninput="drawPick(this.value)" ${q?`value="${esc(q)}"`:''}>
...
function seg2stat(slot,key,opts){const w=cfgOf(slot);return `<div class="seg2" style="flex-wrap:wrap;max-width:340px">${opts.map(o=>statS2(o,w[key]===o,`cset('${slot}','${key}','${o}')`)).join('')}<div class="s2 ${!w[key]?'on':''}" onclick="cset('${slot}','${key}','')">—</div></div><div class="gemlbl"><i class=vgi-gem></i> Gemmes</div>`;}
