...
function load(){try{const r=JSON.parse(JSON.stringify(window.__AGSTATE||null));if(r&&r.members){r.prices=r.prices||{};r.debts=r.debts||[];r.cart=r.cart||{};r.farm=r.farm||{};r.overrides=r.overrides||{};r.recipes=r.recipes||{};r.cats=r.cats||[];r.hiddenCats=r.hiddenCats||[];r.catAssets=r.catAssets||{};if(r.tab==='dj')r.tab='bank';if(r.tab==='obj')r.tab='craft';if(r.tab==='shop')r.tab='set';return r;}}catch(e){}
  return{members:['Commun'],cur:'Commun',inv:{Commun:{}},custom:[],hidden:[],log:[],closed:{},farm:{},prices:{},debts:[],cart:{},overrides:{},recipes:{},cats:[],hiddenCats:[],catAssets:{},shopMember:'',tab:'bank'};}
...
function catIcon(cat){const c=cat.trim();if(S.catAssets&&S.catAssets[c])return '<img src="'+S.catAssets[c]+'" style="width:18px;height:18px;border-radius:4px;object-fit:cover;vertical-align:middle" alt="">';if(c.startsWith('Stuff'))return c.includes('Éternel')?'🟠':c.includes('Shaitan')?'🟥':c.includes('Dryades')?'🟩':'🟣';if(c.startsWith('Armes'))return '⚔️';if(c==='Bijoux')return'💍';if(c==='R1')return'🔵';if(c==='R2')return'🟡';if(c==='Ressource')return'⛏️';if(c.startsWith('Carte'))return'🃏';if(c.startsWith('Butin'))return'🎁';return'📦';}
...
function bankBody(){const cats=sortByOrder(catalog());const isTotal=S.cur==='__total__';
  ...
  let body='';order.forEach(cat=>{let list=byCat[cat];if(!list||!list.length)return;
    ...
    body+=`<div class="catblk ${closed}"><div class="cathead" onclick="togC('${sq(cat)}')"><span class="ci">${catIcon(cat)}</span><span class="ct">${esc(cat.trim())}</span><span class="meta"><span class="pill">${list.length}</span><span class="pill">${fmt(sums)} u.</span><span class="chev">▾</span></span></div><div class="catbody">${list.map(it=>itemRow(it,isTotal)).join('')}</div></div>`;});
...
function aiCatHTML(){var cats=allCats();var ed=canEdit();return `<div class="hint">Crée et gère les catégories — elles apparaissent dans le menu déroulant de l'onglet Item.${ed?'':' <b>Édition réservée au rôle Vanguard.</b>'}</div>
